...
function dropDown() {
    ...
    d3.json(district_bound).then(function (data) {
        console.log(data);
        let districtNumber = data.district;

        districtNumber.forEach((district_bound) => {
            objectDropDown
                .append("option")
                .text(district_bound)
                .property("value", district_bound);
        });
        let firstNumber = districtNumber[0];
        cases(firstNumber);
...
function optionChanged(newNumber) {
    // Fetch new data each time a new district is selected
    cases(newNumber);
}
